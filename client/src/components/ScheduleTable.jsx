	...
	return (
		<>
			...
			>
								...
								<button
									...
									className={`flex flex-col items-center overflow-y-scroll row-span-${getRowSpan(
										...
										!isPast(new Date(showtime.showtime))
											...
											: `bg-red-200  ${
													auth.role === 'admin' ? 'hover:bg-red-300' : 'cursor-not-allowed'
											  }`
									} ${!showtime.isRelease && 'ring-2 ring-inset ring-red-800'}`}
									onClick={() => {
				...
				{showtimeCount === 0 && (
					<div className="col-span-full row-start-3 flex items-center justify-center text-xl font-semibold text-red-700">
				...
				{cinema.theaters.map((theater, index) => (
					<div
						key={index}
						className="sticky top-0 row-span-1 row-start-1 flex flex-col items-center justify-center bg-gradient-to-br from-red-800 to-red-700 py-1 text-white"
					>
