...
const Showtimes = ({ showtimes, movies, selectedDate, filterMovie, showMovieDetail = true }) => {
	...
	if (Object.keys(sortedShowtimes).length === 0) {
		return <p className="text-center text-red-600">There are no showtimes available</p>
	}
	return (
		<>
					...
					sortedShowtimes[movie._id] && (
						<div key={index} className="flex items-center bg-white p-4 shadow-md rounded-md">
							...
							<div className="mr-4 flex flex-col gap-2 pb-4 pt-2">
								{showMovieDetail && (
									<div>
										<h4 className="text-2xl font-semibold text-red-600">{movie.name}</h4>
											...
											<button
												...
												className={
													isPast(new Date(showtime.showtime))
														? `flex items-center gap-1 rounded-md bg-red-100 px-2 py-1 text-lg text-red-900 ring-1 ring-inset ring-red-300 drop-shadow-sm ${
														...
														? 'flex items-center gap-1 rounded-md bg-red-600 px-2 py-1 text-lg text-white drop-shadow-sm hover:bg-red-500'
														: 'flex items-center gap-1 rounded-md bg-red-500 px-2 py-1 text-lg text-white drop-shadow-sm hover:bg-red-400'
												}
												onClick={() => {
