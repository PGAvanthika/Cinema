...
const TheaterListsByCinema = ({ cinemas, selectedCinemaIndex, setSelectedCinemaIndex, fetchCinemas, auth }) => {
	...
	return (
		<div className="mx-4 h-fit rounded-md bg-white text- red-900 drop-shadow-md sm:mx-8">
			<div className="flex items-center justify-center gap-2 rounded-t-md bg-gradient-to-br from-red-700 to-red-600 px-2 py-1.5 text-center text-2xl font-semibold text-white sm:py-2">
				{isEditing ? (
					<input
						title="Cinema name"
						type="text"
						required
						autoFocus
						className={`flex-grow rounded border border-white bg-gradient-to-br from- red-900 to- red-800 px-1 text-center text-2xl font-semibold drop-shadow-sm sm:text-3xl ${
						...
						{isEditing ? (
							<form onClick={handleSubmitName(onEditCinema)}>
								<button
									...
									className="flex w-fit items-center gap-1 rounded-md bg-gradient-to-r from-white-600 to-blue-500  py-1 pl-2 pr-1.5 text-sm font-medium text-white hover:from-white-500 hover:to-blue-400"
									onClick={() => {
										SetIsEditing(false)
									}}
								>
			...
			<div className="flex flex-col gap-6 p-4 sm:p-6 overflow-y-auto">
				...
				<form className="flex flex-col gap-4" onSubmit={handleSubmit(onIncreaseTheater)}>
					...
					{auth.role === 'admin' && (
						<div className="flex w-full flex-wrap justify-between gap-4 rounded-md bg-gradient-to-br from-white-100 to-white p-4">
							...
							<div className="flex grow flex-col gap-4 sm:justify-end md:flex-row">
								...
								<div className="flex grow md:grow-0">
									<div className="flex flex-col items-center justify-center gap-1 rounded-l bg-gradient-to-br from- red-800 to- red-700 p-1 text-white">
									...
									<button
										...
										className="flex grow items-center justify-center whitespace-nowrap rounded-r bg-gradient-to-r from-white-600 to-blue-500 px-2 py-1 font-medium text-white drop-shadow-md hover:from-
										-500 hover:to-blue-400 disabled:from-slate-500 disabled:to-slate-400 md:grow-0"
										type="submit"
									>
